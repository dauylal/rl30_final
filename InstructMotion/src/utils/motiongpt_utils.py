...
def build_dpo_dataset(
    ...
    if sanity_check:
        ...
        c_before = Counter([ex["chosen"][-1]["choice"] for ex in dataset])
        print("before", c_before)

    # ========== flip ==========
    if flip_ratio > 0:        
        def flip_choice(example, idx):
            import random
            rng = random.Random(seed + idx)

            choice = example["chosen"][-1]["choice"]
            if choice == "skipped":
                return example

            if rng.random() < flip_ratio:
                new_choice = "sample_2" if choice == "sample_1" else "sample_1"
                new_chosen = [item.copy() for item in example["chosen"]]
                new_chosen[-1]["choice"] = new_choice
                example["chosen"] = new_chosen

            return example
        
        dataset = dataset.map(flip_choice, with_indices=True)
        if sanity_check:
            from collections import Counter
            c_after = Counter([ex["chosen"][-1]["choice"] for ex in dataset])
            print("after", c_after)
        print(f"✓ Flipped {flip_ratio*100}% of choices")
