...
class DPOTrainer(Trainer):
    ...
    def concatenated_inputs(
        ...
        label_pad_token_id: int = -100,
        padding_value: int = 32000, #for phi3
        ...
        """
        #这个函数的作用是将chose和reject进行合并，如果原来输入是bsz, seq. 输出就是bsz*2, seq
        concatenated_batch = {}
        ...
        #     batch['images'][1] * 2                                
        # ]
        new_images = torch.cat((batch['images'],batch['images']),dim=0)
        new_context_images = torch.cat((batch['context_images'],batch['context_images']),dim=0)
        # new_keypoints = torch.cat((batch['keypoints'],batch['keypoints']),dim=0)
        concatenated_batch['concatenated_images'] = new_images
        concatenated_batch['concatenated_context_images'] = new_context_images
        # concatenated_batch['concatenated_keypoints'] = new_keypoints
    ...
    def concatenated_forward(
        ...
        all_logits, new_labels = model(
            ...
            context_images=concatenated_batch["concatenated_context_images"],
            # keypoints=concatenated_batch["concatenated_keypoints"],
            use_cache=False,
            dpo_forward=True,
        )
