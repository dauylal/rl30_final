...
from new_trl.trainer import DPOTrainer
...
class VideoGPTPlusDPOTrainer(DPOTrainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if 0 and getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            super(VideoGPTPlusDPOTrainer, self)._save_checkpoint(model, trial, metrics)

    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if 0 and getattr(self.args, 'tune_mm_mlp_adapter', False):
            pass
        else:
            super(VideoGPTPlusDPOTrainer, self)._save(output_dir, state_dict)
