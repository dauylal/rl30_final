    ...
    for source in sources:
        for sentence in source:
            ...
            if DEFAULT_IMAGE_TOKEN in sentence['value'] or DEFAULT_VIDEO_TOKEN in sentence['value']:
                ...
                IMAGE_TOKEN_NUM = sentence['value'].count(DEFAULT_IMAGE_TOKEN) #0
            ...
            replace_token, vid_replace_token = DEFAULT_IMAGE_TOKEN, DEFAULT_IMAGE_TOKEN * NUM_FRAMES #16
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:  # Alternatively if 'context_image' in instances[0]
            ...
            keys = [instance['keypoints'] for instance in instances] #each : [1,8,37] tensor. if k!=1 [k,8,37]
            ...
            # batch['key_mask'] = torch.cat(key_mask,dim=0) #[bsz, 16,17,3]
            if keys[0] is not None:
                batch['keypoints'] = torch.cat(keys,dim=0) #[bsz, 16,37]
            else:
