...
from mcdpo.train.trainer import VideoGPTPlusTrainer
from mcdpo.train.dpo_trainer import VideoGPTPlusDPOTrainer
from mcdpo import conversation as conversation_lib
from mcdpo.model import *
from mcdpo.mm_utils import tokenizer_image_token
from mcdpo.config import DataConfig
from PIL import Image
import random
import numpy as np
from mcdpo.model.dataloader import _get_rawvideo_dec,build_clip_model,_get_fixed_text_fea
import os
# from mcdpo.model.wham_dataloader.detector import DetectionModel
# from mcdpo.model.wham_dataloader.extractor import FeatureExtractor
# from mcdpo.model.wham_dataloader.normalizer import Normalizer
# from mcdpo.model.wham_dataloader.smpl import SMPL
# import mcdpo.model.wham_dataloader.constants as _C
import sys

from mcdpo.model.internvideo_1.modules.tokenization_clip import SimpleTokenizer as ClipTokenizer
...
def preprocess(
    ...
    """
    # print(conversation_lib.default_conversation.sep_style)  SeparatorStyle.MPT
    # print(conversation_lib.default_conversation.version) phi3
    # exit()
    ...
    if conversation_lib.default_conversation.version == "phi3":
        return preprocess_phi3(sources, tokenizer, has_image=has_image,k=k) #text only mode run this
    # add end signal and concatenate together
    conversations = []
    for source in sources:
...
class LazyDPODataset(Dataset):
    ...
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer,
        ...
        # self.k = data_args.k #denotes top k videos
        self.k = 1
    ...
    def _get_motion_motionbert(self,video_paths,max_frames):
        ...
        for i,video_file_path in enumerate(video_paths):
            ...
            if cur_keypoints.shape[0]< max_frames: #batch 
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources:
            ...
            if self.k!=1:
                ...
                #finally, frames and context_images a list of tensors size: 16*k
            else:
        ...
        # data_dict['init_kp'] = init_kp
        # data_dict['init_smpl'] = init_smpl
        # data_dict['key_mask'] = key_mask
        # data_dict['fixed_text_embeds'] = fixed_text_embeds
        return data_dict
...
class DataCollatorForDPODataset(DPODataCollatorWithPadding):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:  # Alternatively if 'context_image' in instances[0]
            images = [instance['image'] for instance in instances] #if self.k!=1. bsz 个16*k
            ...
            if all(x is not None and x.shape == images[0].shape for x in images):
                batch['images'] = torch.stack(images) #[bsz*16*k,3,224,224] #1-1,1-2,1-3, ... 2-1,2-2, 2-3 ... 
                batch['context_images'] = torch.stack(context_images)

            else:
                batch['images'] = images
                batch['context_images'] = context_images
            # batch['key_mask'] = torch.cat(key_mask,dim=0) #[bsz, 16,17]
            # batch['keypoints'] = torch.cat(keys,dim=0) #[bsz, 16,17，3]
