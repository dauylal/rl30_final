...
import random
import numpy as np
from mcdpo.model.dataloader import _get_rawvideo_dec
...
class ModelArguments:
    ...
    tune_motion_mm_mlp_adapter: bool = field(default=False)
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_motion_mm_mlp_adapter", False):
        # Only save Adapter
        keys_to_match = ['motion_mm_projector']
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_motion_motionbert(self,video_paths,max_frames):
        ...
        return torch.from_numpy(np.array(all_keypoints)) #[1,16,17,3]
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:
            ...
            # keys = [instance['keypoints'] for instance in instances] #each : [1,8,37] tensor
            # init_kp = [instance['init_kp'] for instance in instances] #each: [1,1,88]
            # init_smpl = [instance['init_smpl'] for instance in instances] # each [1,1,24,6]
            # key_mask = [instance['key_mask'] for instance in instances] #each [1,8,17]
            new_images = []
            for image in images:
            ...
            images = new_images # a list of  tensors [3,224,224] list size is bsz*16 (frames)
...
def train():
    ...
    model.config.tune_motion_mm_mlp_adapter = training_args.tune_motion_mm_mlp_adapter = model_args.tune_motion_mm_mlp_adapter
    ...
    if model_args.tune_image_mm_mlp_adapter:
        ...
        for p in model.get_model().image_mm_projector.parameters():
            p.requires_grad = True
    if model_args.tune_motion_mm_mlp_adapter:
        model.requires_grad_(False)
