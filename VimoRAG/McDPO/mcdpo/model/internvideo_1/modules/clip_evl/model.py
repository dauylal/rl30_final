...
class CLIP(nn.Module):
    ...
    def encode_video(self, video, return_all_feats=False, masked_indices=None, mode="video"):
        ...
        if return_all_feats:
            x, feats = feats
        #feats: 257,bsz, 12, 1024],   如果是image模式就是 257, bsz, 1, 1024
        ...
        if mode=='video':
            bsz = feats.size()[1]
            hidden = feats.size()[-1]
            feats = self.visual_ln_post(feats[1:,:,:,:]).reshape(bsz,-1,hidden)
        else:
            feats = self.visual_ln_post(feats.squeeze(2)).permute(1,0,2) #[bsz,257,1024]
