...
class Transformer(nn.Module):
    ...
    def forward(self, x, mode='video', return_all_feats=False):
        ...
        if mode == 'video':
            ...
            T_down = 1
        L, NT, C = x.shape # 257, 384,1024,       T_down: 12
        ...
        for i, resblock in enumerate(self.resblocks):
            if self.use_checkpoint and i < self.checkpoint_num[0]:  #[24]
                x = resblock(x, self.T, use_checkpoint=True)
            else:
                x = resblock(x, T_down)
            if i in self.return_list: #[20,21,22,23]
                # continue
                j += 1
                tmp_x = x.clone()
        ...
        weight = self.sigmoid(self.balance) # weight: [1024]
        residual = x.view(L, N, T_down, C)[0].mean(1)  # L, N, T, C.  [bsz, 1024].  原来是pooling之后  +cls token,那么现在应该是。不pooling +cls token
        # return self.proj((1 - weight) * cls_token[0, :, :] + weight * residual).  cls_token大小是[1,bsz,1024]，也就是一个video只有一个，处于257个patch中的第一个
        feats = (1 - weight) * cls_token[0, :, :] + weight * residual
        # import ipdb;ipdb.set_trace()
        x = x.view(L,N,T_down,C)
        final = (1-weight) * cls_token.unsqueeze(2).repeat(L,1,T_down,1) + weight * x

        # final_x[0,:,:,:] = feats.unsqueeze(1).expand(N, T_down, C). 有bug，这里所有的token都是一样的，最后用的就是[0,:,:,:]，后果是每个视频的所有序列都一样, final_x的维度是[257, bsz, 12, 1024]
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, mode='video', return_all_feats=False):
        x = self.conv1(x)  # shape = [*, width, grid, grid]. [bsz, 1024, 12, 16,16]
        N, C, T, H, W = x.shape
        x = x.permute(0, 2, 3, 4, 1).reshape(N * T, H * W, C)
        # x up.  [bsz*12, 16*16,1024]
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        x = x + self.positional_embedding.to(x.dtype)
        x = self.ln_pre(x)
        #up: [384,257,1024].   down:  [257,384,1024]
        x = x.permute(1, 0, 2)  # NLD -> LND
        out = self.transformer(x, mode=mode, return_all_feats=return_all_feats)
        return out
        #tensor 1: [bsz, 1024].  tensor 2: [257,bsz, 12, 1024]， 这里最后用的是tensor1，tensor1是由CLS token计算而来
