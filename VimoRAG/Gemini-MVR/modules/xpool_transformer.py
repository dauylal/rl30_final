...
class MultiHeadedAttention_self(nn.Module):
    def __init__(self, embed_dim,num_mha_heads):
        super(MultiHeadedAttention_self, self).__init__()
        # self.embed_dim = embed_dim
        # self.num_heads = num_mha_heads
    ...
    def forward(self, video_embeds):
        """
        Input
            video_embeds: bsz x num_frames x embed_dim
        Output

        """
        return self.attn(video_embeds,video_embeds,video_embeds)[0]
...
class Transformer_self(nn.Module):
    ...
    def __init__(self, embed_dim,num_mha_heads):
        ...
        self.self_attn = MultiHeadedAttention_self(embed_dim,num_mha_heads)
    ...
    def forward(self, video_embeds):
        ...
        # num_vids x num_texts x embed_dim
        attn_out = self.self_attn(video_embeds)
