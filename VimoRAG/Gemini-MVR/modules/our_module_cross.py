...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, q: torch.Tensor,k: torch.Tensor,v: torch.Tensor, attn_mask: torch.Tensor):
        # attn_mask_ = attn_mask.repeat_interleave(self.n_head, dim=0)
        # return self.attn(q, k, v, need_weights=False, key_padding_mask=attn_mask)[0]
        true_mask = torch.logical_not(attn_mask)
        return self.attn(q, k,v, need_weights=False, key_padding_mask=true_mask)[0]

    def forward(self, para_tuple: tuple):
        # x: torch.Tensor, attn_mask: torch.Tensor
        # print(para_tuple)
        q,k,v, attn_mask = para_tuple
        # try:
        q = q + self.attention(self.ln_1(q),self.ln_1(k),self.ln_1(v), attn_mask)
...
class Transformer(nn.Module):
    ...
    def forward(self, q: torch.Tensor,k:torch.Tensor, v:torch.Tensor, attn_mask: torch.Tensor):
        return self.resblocks((q,k,v, attn_mask))[0]
